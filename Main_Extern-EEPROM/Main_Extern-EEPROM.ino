...
void setup()
{
    ...
    const word address = 4095; //Adresse 0-4095
    byte data = 0x12
    
    Serial.println("Write byte to EEPROM memory...");
    eeprom.writeByte(address, data); //Max-Adresse: 4095
    
    
    delay(10); //laut Datenblatt - Write cycle time = 5ms
    
    Serial.println("Read byte from EEPROM memory...");
    byte dataR = eeprom.readByte(adress); //Max-Adresse: 4095
    
    // Print read byte.
    Serial.print("Read byte = 0x");
    Serial.print(dataR, HEX);
    Serial.println();
}
